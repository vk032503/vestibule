"""Real `_TableBackend` translation layer for `TableStorageIndexRegistry` (REQ-011).

Split out of `table_storage_registry.py` to keep that module's own testable
orchestration/retry logic under the house rules' module-size guidance (mirrors
`vestibule.scenario.stores._azure_table_backend`'s precedent for the same reason).
...
only from `TableStorageIndexRegistry.__init__` when no test `_backend` was injected.
...
def _load_real_backend(connection_string: str, table_name: str) -> _TableBackend:
    ...
    try:
        ...
        raise ProvisioningError(
            ...
            f"(pip install vestibule[{_EXTRA_NAME}]) to use TableStorageIndexRegistry",
...
class _RealTableBackend:
    """Production `_TableBackend`: translates this registry's calls to the real SDK.
