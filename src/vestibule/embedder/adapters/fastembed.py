...
`nomic-ai/nomic-embed-text-v1.5` (768 native dimensions, Matryoshka-capable, ~274MB) —
the `nomic-ai/` namespace prefix is required; `fastembed.TextEmbedding.list_supported_models()`
does not register the unprefixed name. `fastembed` is an optional dependency (the
`local` extra) — imported lazily inside `__init__` so
